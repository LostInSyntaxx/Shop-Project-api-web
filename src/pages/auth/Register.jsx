...
// Schema Validation
const passwordSchema = z.string().min(6, { message: "รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร" })
...
const Register = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await axios.post("https://shop-main-api.vercel.app/api/register", { ...data, recaptcha: recaptchaValue });
            Swal.fire({ title: "สมัครสมาชิกสำเร็จ!", text: "ยินดีต้อนรับเข้าสู่ระบบ", icon: "success", timer: 2000, showConfirmButton: false });
            ...
            <div className="bg-[#101010] p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl" data-aos="fade-up">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="relative">
                        ...
                        <input {...register("email")} className="input input-bordered w-full pl-10 bg-white/10 text-white" type="email" placeholder="อีเมลของคุณ" />
                    ...
                    <div className="relative">
                        ...
                        <input {...register("password")} className="input input-bordered w-full pl-10 pr-10 bg-white/10 text-white" type={showPassword ? "text" : "password"} placeholder="รหัสผ่าน" onChange={handlePasswordChange} />
                        ...
                        <div className="mt-1 flex gap-1">
                            {[...Array(5)].map((_, i) => (
                                <span key={i} className={`w-1/5 h-[6px] rounded-lg transition-all ${passwordStrength === -1 ? "bg-gray-500" : passwordStrength >= 4 ? "bg-green-400" : passwordStrength >= 2 ? "bg-yellow-400" : "bg-red-400"}`} />
                            ))}
                        </div>
                    ...
                    <div className="relative">
                        <FontAwesomeIcon icon={faLock} className="absolute left-3 top-3 text-white/50" />
                        <input {...register("confirmPassword")} className="input input-bordered w-full pl-10 pr-10 bg-white/10 text-white" type={showConfirmPassword ? "text" : "password"} placeholder="ยืนยันรหัสผ่าน" />
